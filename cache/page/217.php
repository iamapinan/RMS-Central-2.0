...
echo '<div class="display_0"><h2 class="title">คู่มือและวีดีโอสอนการใช้งาน <span class="page-option">
...
<option value="" selected >ดูทั้งหมด</option>';
		$sql = mysql_query('select * from '.conf('table_prefix').'_subcat where catid=19');
...
$SQLDB0 = mysql_query('SELECT * FROM '.conf('table_prefix').'_subcat WHERE `catid`=19  ORDER BY scid, type ASC');
while($fetch0 = mysql_fetch_array($SQLDB0))
{
	...
	WHERE  `desc`<>"" AND `desc` LIKE  "%'.$fetch0['scid'].'%" AND `group`=19');
