...
{
		$name = md5(basename($img));
		//$fileTypes = substr($img, strrpos($img,'.'), strlen($img)-1); // Get the extension from the filename.
		//$fileTypes = strtolower($fileTypes);

		if (preg_match('/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?/', $img))
		{
		...
		$imgex = basename($img);
		//$fileTypes = substr($imgex, strrpos($imgex,'.'), strlen($imgex)); // Get the extension from the filename.
		//$fileTypes = strtolower($fileTypes);

		$location = $_SERVER['DOCUMENT_ROOT'].'/cache/'.$name;
		...
		if($valid==true)
		{
			...
			file_put_contents($gimg, file_get_contents($url));

			$result =  '/cache/'.$name.$fileTypes;
			return $result;
		}
		else
			return $img;
...
//Set data for doing next step.
$_GET['image'] = cache_image($_GET['image']);
...
define('DOCUMENT_ROOT',			str_replace('private_html','public_html',$_SERVER['DOCUMENT_ROOT']));
...
{
	$_GET['image'] = cache_image('/data/thumb/thumb.png');
}
