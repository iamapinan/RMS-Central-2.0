...
while($std = mysql_fetch_assoc($strSQL)){
	...
	$course['student'][$std->uid]['photo'] = ($student_data['avatar']!='') ? conf('url').'user/'.$student_data['user'].'/'.$student_data['avatar'] : '';
	$course['student'][$std->uid]['gender'] = $student_data['gender'];
	$course['student'][$std->uid]['grade'] = $student_data['grade'];
	$course['student'][$std->uid]['class'] = $student_data['class'];
	$course['student'][$std->uid]['citizid'] = $student_data['citizen_id'];
